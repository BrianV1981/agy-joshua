...
To update an item's status, you need the Project ID, Item ID, and the ID of the Status field.
1. Fetch Project ID: `gh project view <PROJECT_NUMBER> --owner <OWNER> --format json`
2. Fetch Field ID for "Status": `gh project field-list <PROJECT_NUMBER> --owner <OWNER> --format json`
3. Update Status: 
   ```powershell
...
```powershell
gh project item-edit --id <ITEM_ID> --field-id <STATUS_FIELD_ID> --project-id <PROJECT_ID> --text "Done"
```

## 4. Blocked
When waiting on the Operator, DNS changes, or external factors:
```powershell
gh project item-edit --id <ITEM_ID> --field-id <STATUS_FIELD_ID> --project-id <PROJECT_ID> --text "Blocked"
```
