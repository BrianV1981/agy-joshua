...
*   **The Golden Rule:** Always spawn a new agent BEFORE compression is necessary.
*   **The Sprint:** Treat terminal sessions like sprint cycles, not marathons. When an agent has completed a distinct logical phase (or begins to thrash), it must execute the `aim-handoff` skill to teleport its specific "Will" (via the `HANDOFF.md`) to a fresh, unburdened agent session. This ensures Senior-level precision is maintained indefinitely.