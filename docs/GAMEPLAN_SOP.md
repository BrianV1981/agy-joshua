...
This document defines the strict, high-fidelity structure required for all `HANDOFF.md` files generated by the `aim-handoff` skill.

## 1. The Rule of Epistemic Certainty
The outgoing agent must act as a precise tactician. The incoming agent will wake up with a blank context window. The `HANDOFF.md` document is their only link to the current mission state. It must provide immediate, actionable, and localized context.

## 2. Mandatory Gameplan Structure
Every `HANDOFF.md` MUST contain the following 5 sections, formatted exactly as shown:
...
*   **Requirements:**
    ...
    *   **Primary Files:** List the absolute or relative paths to the specific files you were actively modifying.
